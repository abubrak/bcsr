...
> **注意：** `coreset` 方法（基于 NTK）未在此代码库中实现，实验只包含 `bcsr`（论文方法）和 `uniform`（随机采样 baseline）。
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--methods` | all | 选择方法 [bcsr, uniform, all] |
...
| 方法 | 单个实验 | 18组实验（串行） | 18组实验（2并行） |
|------|----------|------------------|-------------------|
| bcsr | ~2小时 | ~36小时 | ~18小时 |
| uniform | ~1小时 | ~18小时 | ~9小时 |
