...
def save_np_arrays(data, path):
    np.save(file=path, **data)

def setup_experiment(experiment, args):
    ...
    if 'mixture' in args.dataset:
        init_model = ResNet18(nclasses=83, config=config)
    else:
        init_model = ResNet18(config=config) if 'cifar' in args.dataset else MLP(config)
    save_model(init_model, '{}/init.pth'.format(args.exp_dir))
